...
function renderPronos() {
  ...
  // Mode courant : 'participant' ou 'match'
  let mode = 'participant';

  function applyMode() {
    const modeParticipant = $('#pronos-mode-participant');
    const modeMatch       = $('#pronos-mode-match');
    ...
    if (mode === 'participant') {
      modeParticipant.classList.add('active');
      ...
      modeParticipant.classList.remove('active');
  ...
  $('#toggle-compare').addEventListener('change', () => {
    if (mode === 'participant') renderSelected();
    else renderMatchSelected();
  });
    ...
    .map(p => {
      ...
      return `
        <tr class="${resultClass}">
          <td class="pm-nom">${escapeHtml(p.nom)}</td>
